...
classeIndex<-which(names(crossvSet)=="classe")
mod_raf_crossv<-train(classe~., method="rf", data=crossvSet, trControl=crossVal)
pred_raf_crossv<-predict(mod_raf_crossv, crossvSet)
#pred_raf_crossv<-predict(mod_raf, crossvSet[ ,-classeIndex])
confMat<-confusionMatrix(crossvSet$classe, pred_raf_crossv)
accu_crossv<-confMat$Accuracy
```
```{r}
1.0-accu_crossv
```
...
# Github repo with your R markdown and compiled HTML file describing your analysis. 
# Constrain the text of the writeup to < 2000 words
